...
estproc_LVCF_mul <- function(data, n, nknots, norder, s, h, initpara=NULL) {
  ...
  kerval <- tibble(dist=kervalinner,id=as.numeric(id))%>%group_by(id)%>%summarise(indicator=ismin(dist),.groups = "keep")%>%pull(indicator)
  ...
  B <- function(beta, gamma) {
    ...
    outerproducinner1[is.na(outerproducinner1)] <-0
    bb1=outerproducinner1*  kerval * delta * ( (X-obs_times)>0 )
    bb1=tibble(a=bb1 ,id=as.numeric(id)) %>% group_by(id) %>%  summarise( colSums(a),.groups="keep")
    nc=ncol(data$covariates)
    ...
    b_inner <- function(tt, beta, gamma) {
      ...
      for( jj in 1:nc){
        temp1  <-
          ...
          kerval_tt*outer(as.vector(S1_tt[,jj]),covariates[,jj],"-") 
        temp1[is.na(temp1)] <-0
        r1=tibble(a=t(temp1) ,id=as.numeric(id)) %>% group_by(id) %>%  summarise( colMeans(a),.groups = "keep")
...
estproc_LVCF_mulboot <- function(data, n, nknots, norder, s, h, initpara=NULL) {
  gammap<- nknots+norder-1
  X <- data$X
  id <- data$id1
  covariates <- data$covariates 
...
boot<-function(data, n, nknots, norder, s, initpara=NULL){
  idd=data$id
  re1=re2=NULL
  for(i in 1:30){
    data2 <-
      replicate(
        n,
        data[which(idd==sample(1:n,size=1)),],
        simplify = F
      )
    a=data2%>% bind_rows(.id = "id1")
    re=estproc_LVCF_mulboot(a, n, nknots, norder, s, 1)[1:2]
    re1=c(re1,re[1])
    re2=c(re2,re[2])
  }
  c(sqrt(var(re1)),sqrt(var(re2)))
